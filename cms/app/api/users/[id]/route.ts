 ...
 *
 * As with the collection route, there is no session check — this app has no
 * sign-in. See the README's "Security" note.
 *
...
export async function PUT(request: Request, ctx: RouteContext<"/api/users/[id]">) {
  try {
    ...
    // Demoting the only admin would lock everyone out of the public site's
    // admin-only pages permanently.
    if (update.role === "user") {
